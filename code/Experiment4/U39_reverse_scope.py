...
def dict_words():
  ...
  markers = ['gu', 'sa', 'ti']
  number_signal = [random.choice(markers)]*4
  markers.pop(markers.index(number_signal[0]))    
  case_signal = markers 
...
#change: function developed for scope
def assign_patientMarker():
  global d_words
  participants = ['burglar','chef','cowboy','waitress']
  markers = ['patient1' if d_words[part] in ['nork','vaem'] else 'patient2' for part in participants]
  d_patientMarker = OrderedDict(zip(participants,markers))
  return d_patientMarker

d_patientMarker = assign_patientMarker()


#changed for scope
def dict_sentences(noun_position):
  global d_words
  global d_patientMarker  
  ...
  if noun_position == 'initial':
    for meaning in list_meanings_sentences_unmarked: meaning.append(d_patientMarker[meaning[-2]])
    for meaning in list_meanings_sentences_marked: meaning.insert(-1,d_patientMarker[meaning[-2]])
  elif noun_position == 'final':
    for meaning in list_meanings_sentences_unmarked: meaning.insert(-2,d_patientMarker[meaning[-1]])
    for meaning in list_meanings_sentences_marked: meaning.insert(-1,d_patientMarker[meaning[-1]])
  ...
  return dict_sentences, d_words, d_patientMarker
...
def sentence2text(sentence):
  ...
  if '_' not in sentence: 
    ...
    if len(sentence_list)==2: 
      ...
      sentence_text = ' '.join([dict_sentence_text[i] for i in sentence_list[:2]])+' '+''.join([dict_sentence_text[i] for i in sentence_list[2:]])
    else:
      len_words=[len(dict_sentence_text[i]) for i in sentence_list[2:]]
...
#developed for scope
def groupMeanings_byPatientparticipant(list_meanings):
  participants = ['burglar','chef','cowboy','waitress']
  meaningsByParticipant = [[meaning for meaning in list_meanings if participant in meaning[-2:]]for participant in participants]
  meaningsByParticipant = [random. sample(meanings, len(meanings)) for meanings in meaningsByParticipant]
  return OrderedDict(zip(participants,meaningsByParticipant))

#developed for scope
def balance_remainder(list_meanings, remainder):
  global d_patientMarker
  participants = ['burglar','chef','cowboy','waitress']
  if remainder == 1:
    rest_meanings = random.sample(list_meanings, remainder)
  else:
    rest_meanings = random.sample(list_meanings, remainder)
    patients = [[part for part in meaning[-2:] if part in participants] for meaning in rest_meanings]
    rest_meanings_markers = [d_patientMarker[patient[0]] for patient in patients]
    while all(marker in rest_meanings_markers for marker in ['patient1', 'patient2'])==False:
      rest_meanings = random.sample(list_meanings, remainder)
      patients = [[part for part in meaning[-2:] if part in participants] for meaning in rest_meanings]
      rest_meanings_markers = [d_patientMarker[patient[0]] for patient in patients]
  return rest_meanings

#developed for scope
def balanced_meaningParticipants(list_meanings, number_meanings):
  meaningsByParticipant = groupMeanings_byPatientparticipant(list_meanings)
  numberPerParticipant = number_meanings/4
  remainder = number_meanings % 4
  balanced_meanings = list(itertools.chain(*[random.sample(meaningsByParticipant[participant],numberPerParticipant) for participant in meaningsByParticipant.keys()]))
  if remainder != 0:
    rest_meanings = balance_remainder(list_meanings, remainder)
    while any(meaning in balanced_meanings for meaning in rest_meanings):
      rest_meanings = balance_remainder(list_meanings, remainder)
    balanced_meanings = balanced_meanings+rest_meanings
  else: 
    balanced_meanings = balanced_meanings
  return balanced_meanings

# meaning_list = get_meanings('initial')[0]
# d_bygroup = get_meaning_groups(meaning_list)
# m = d_bygroup['Case']+d_bygroup['NNumNCase']
# print balanced_meaningParticipants(m, 16)
...
def get_mixed_simpleSentences(list_meanings):
  ...
  case1_sample = balanced_meaningParticipants(case1, 24)
  case2_sample = balanced_meaningParticipants(case2, 24)
  case = case1_sample+case2_sample
  case_exposure = balanced_meaningParticipants(case1_sample,12)+balanced_meaningParticipants(case2_sample,12)
  case_comprehension = [meaning for meaning in case if meaning not in case_exposure]
  case_typing = random.sample(case1_sample,4) + random.sample(case2_sample,4)
  case1_sample_testing = [meaning for meaning in case1 if meaning not in case1_sample]+balanced_meaningParticipants(case1, 6)
  case2_sample_testing = [meaning for meaning in case2 if meaning not in case2_sample]+balanced_meaningParticipants(case2, 6)
...
def meanings_SimplePhases(condition,list_meanings,dict_meanings_byGroup,mixed_case):
  ...
  if condition == 'regularisation':    
    ...
    case1 = case[3][0]
    case2 = case[3][1]
    case1_comp = balanced_meaningParticipants(case[4][0], 12)
    case2_comp = balanced_meaningParticipants(case[4][1], 12)
    case1_type = balanced_meaningParticipants(case1,8)
    case2_type = balanced_meaningParticipants(case2,8)
...
#should be changed for scope (this is from freq)
def meanings_ComplexPhases(condition,list_meanings,dict_meanings_byGroup,mixed_case):
  ...
  numcase_typing = balanced_meaningParticipants(numcase[:20],4)
  case1 = simple_case[0]
  case2 = simple_case[1]
  case1_exp = case1[:6]
  case2_exp = case2[:6]
...
def meanings_testing(condition,list_meanings,dict_meanings_byGroup,numcase_training):
  ...
  numcase_sample = balanced_meaningParticipants(numcase,12)
  #num_sample = random.sample(num, 6)
  case1_sample = balanced_meaningParticipants(case1, 6)
  case2_sample = balanced_meaningParticipants(case2, 6)
...
def set_outputCSV(path,ID):
    ...
    if os.path.isfile(file)==False:
      with open(file,'wb') as f:
          w = csv.writer(f)
          w.writerow(['ID','Condition','MajorityOrder','NounPosition','markerNum', 'markerCase', 'Phase', 'Meaning', 'NounItem','Noun','InputSentence', 'Selection', 'TypedResponse', 'SpokenResponse', 'AnswerQuest', 'Trial', 'date&time'])
      f.close()
    else:
...
def meaning_marker(myWin, markerInfo):
  marker = markerInfo[0]
  if type(marker)==list:
    question = "What do '%s' and '%s' describe in Tak Pisan?"%(marker[0],marker[1])
  else:
...
def order_markerNoun(myWin, markerInfo):
  marker = markerInfo[0]
  if type(marker)==list:
    question = "In the descriptions you were trained on, did '%s' and '%s' appear before or after the name of the character they referred to?"%(marker[0],marker[1])
  else:
...
def order_markers(myWin, markerNum, markerCase, noun_position):
  if noun_position == 'initial':
    ...
    direction = 'before'
  question = "And when both '%s' and '%s/%s' appeared %s the name of the character it referred to, did '%s' appear before '%s', after '%s', or both?"%(markerNum, markerCase[0], markerCase[1],direction,markerNum, markerCase, markerCase)
  answer = getAndShow(myWin,question)
  return answer, 'OrderNumCase'


def order_markers_prod(myWin, markerNum, markerCase, noun_position):
  if noun_position == 'initial':
    direction = 'after'
  elif noun_position == 'final':
    direction = 'before'
  question = "When you produced both '%s' and '%s/%s' together %s the name of the character they referred to, did you produce '%s/%s' before '%s', after '%s', or both? Can you tell why?"%(markerNum, markerCase[0], markerCase[1], direction, markerCase[0], markerCase[1], markerNum, markerNum)
  answer = getAndShow(myWin,question)
  return answer, 'OrderNumCase'
...
def post_experimental_questionnaire(myWin,path,ID,condition,condition_order,noun_position, markerNum, markerCase, phase):
  ...
  selection_marker_marker_prod = order_markers_prod(myWin,markerNum,markerCase,noun_position)
  if condition == 'regularisation':
  ...
  if condition == 'regularisation':
    selections = [selection_marker1, selection_marker2, selection_marker1_noun, selection_marker2_noun,selection_marker_marker, selection_marker_marker_prod,selection_frequency,spoken_languages]
  elif condition == 'extrapolation':
    selections = [selection_marker1, selection_marker2, selection_marker1_noun, selection_marker2_noun,selection_marker_marker_prod, spoken_languages]
