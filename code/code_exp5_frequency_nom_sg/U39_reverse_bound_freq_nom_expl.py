...
def setSound(signal,path):
  instr = sound.Sound(path+'U39_stims/sentences/all/'+signal+'.wav')
...
def get_meanings(noun_position):
  ...
  combi_sentences = [meaning for meaning in combi_sentences if meaning[1]!=meaning[3] and all(x in ['sg'] for x in [meaning[2],meaning[4]])==False]
...
def get_meaning_groups(meaning_list):
  Nouns = [meaning  for meaning in meaning_list if len(meaning) == 2]
  Num = [meaning  for meaning in meaning_list if len(meaning) == 1]
  Case = [meaning for meaning in meaning_list if meaning.count('pl')==2]
  NumCase = [meaning for meaning in meaning_list if 'sg' in meaning[:-2]]
  NNumNCase = [meaning for meaning in meaning_list if 'sg' in meaning[-2:]]
...
def dict_words():
  ...
  singular = ['sg']
  case = ['agent']
  verb_signals = ["kikim", "poinim", "straikim"]
  participant_signals = ["negid", "tumbat", "vaem", "nork"]
  group_signal = ['', '']
  ...
  case_signal = random.choice(['gu', 'sa', 'ti'])
  singular_signal = random.choice(['gu', 'sa', 'ti'])
  while levenshtein(case_signal,singular_signal) <= 0: 
    case_signal = random.choice(['gu', 'sa', 'ti'])
  dict_words = OrderedDict(zip(verbs+participants+singular+group+case, verb_signals+participant_signals+[singular_signal]+group_signal+[case_signal]))
  return dict_words
...
def dict_sentences(noun_position): 
  ...
  if noun_position == 'initial':
    for meaning in list_meanings_sentences_unmarked: meaning.insert(3,'agent')
    for meaning in list_meanings_sentences_marked: meaning.insert(2,'agent')
  elif noun_position == 'final':
    for meaning in list_meanings_sentences_unmarked: meaning.insert(1,'agent')
    for meaning in list_meanings_sentences_marked: meaning.insert(2,'agent')
  ...
  if noun_position=='initial':
    list_phrases = [i[:-1] if i[-1]=="_" else i+'_'+d_words['sg'] for i in list_phrases]
    list_sentences_unmarked = [i[:-1] if i[-1]=="_" else i for i in list_sentences_unmarked]
    list_sentences_marked = [i[:-1] if i[-1]=="_" else i for i in list_sentences_marked]
  else: 
    list_sentences_unmarked = [i[1:] if i[0]=="_" else i for i in list_sentences_unmarked]
    list_sentences_marked = [i[1:] if i[0]=="_" else i for i in list_sentences_marked]
    list_phrases = [i[1:] if i[0]=="_" else d_words['sg']+'_'+i for i in list_phrases]
...
def dict_sentence2text():
  # signals = ["kIHkIHm", "pOYnIHm", "strAYkIHm", "nEHZIHd", "tUHmbAXt", "vAEm", "nAOrk", "gUW", "sAA", "tIY"]#, "sUWtIHm",
  signals = ["kikim", "poinim", "straikim", "negid", "tumbat", "vaem", "nork", "gu", "sa", "ti"]# "sutim",
  text_signals = ["kikim", "poinim", "straikim", "negid", "tumbat", "vaem", "nork", "gu", "sa", "ti"]# "sutim",
  d=OrderedDict(zip(signals,text_signals))
  return d

dict_sentence_text=dict_sentence2text()

#changed from unbound
def sentence2text(sentence):
  ...
  if '_' not in sentence: 
    ...
    if len(sentence_list)==2: 
      sentence_text = ''.join([dict_sentence_text[i] for i in sentence_list])
    elif len(sentence_list)==4:
      sentence_text = dict_sentence_text[sentence_list[0]]+' '+''.join([dict_sentence_text[i] for i in sentence_list[1:3]])+' '+dict_sentence_text[sentence_list[3]]
    else:
      len_words=[len(dict_sentence_text[i]) for i in sentence_list[1:-1]]
      if len_words[1:].count(2)==2 or len_words[:2].count(2)==2:
        sentence_text = dict_sentence_text[sentence_list[0]]+' '+''.join([dict_sentence_text[i] for i in sentence_list[1:4]])+' '+''.join([dict_sentence_text[i] for i in sentence_list[4:]])
      else:
        sentence_text = ' '.join([dict_sentence_text[i] for i in sentence_list[:1]])+' '+''.join([dict_sentence_text[i] for i in sentence_list[1:3]])+' '+''.join([dict_sentence_text[i] for i in sentence_list[3:]])    
  return sentence_text

#changed from unbound
def sentence2text_gap(sentence):
  sentence=sentence2text(sentence)
  sentence_list = sentence.split(' ')
  gaps_list = ['-'*len(word) for word in sentence_list]
  if len(sentence_list) > 2:
    sentence_text = sentence_list[0]
...
def meanings_SimplePhases(condition,list_meanings,dict_meanings_byGroup,mixed_case):
  ...
  if condition == 'regularisation':    
    ...
    num_exp = num+random.sample(num, 2)
    num_comp = num*2+random.sample(num, 2)
    num_type = num 
    exposure = case1 + case2 + noun*2 
    comprehension = case1_comp + case2_comp
    typing = case1_type+case2_type+noun
...
def meanings_ComplexPhases(condition,list_meanings,dict_meanings_byGroup,mixed_case):
  ...
  exposure = numcase[:16]*3 + case1_exp + case2_exp + noun
  comprehension = numcase[16:20]*3 + case1_comp+case2_comp
...
def meanings_testing(condition,list_meanings,dict_meanings_byGroup,numcase_training):
  ...
  num_sample = num+random.sample(num, 2)
  case1_sample = random.sample(case1, 6)
  case2_sample = random.sample(case2, 6)
  testing = noun + numcase_sample + case1_sample +numcase_sample+ case2_sample + numcase_sample
  return [testing]

# condition="extrapolation"
# list_meanings=get_meanings("initial")[0]
# dict_meanings_byGroup=get_meaning_groups(list_meanings)

# mixed_case=get_mixed_simpleSentences(list_meanings)
# simple= meanings_SimplePhases(condition, list_meanings,dict_meanings_byGroup,mixed_case)
# complex_phrases=meanings_ComplexPhases(condition, list_meanings,dict_meanings_byGroup,mixed_case)
# numcase_training=complex_phrases[1]
# meanings_testing= meanings_testing(condition,list_meanings,dict_meanings_byGroup,numcase_training)
...
# print dict_meaning_trials(condition,list_meanings)
...
# dict_s=dict_sentences("initial")[0]
# print dict_sentences_training(condition, "unmarked", dict_s)
...
def choose_foilNoun(noun):
  object_signals = ['negid', "tumbat", "vaem", "nork"]
...
def typing_trial(myWin,meaning,sentence):
  ...
  while event.getKeys(keyList=['return'])==[] or text=="" or len(gaps.split(' ')[-1])!=len(text.split(' ')[-1]):
...
def set_outputCSV(path,ID):
    ...
    if os.path.isfile(file)==False:
      with open(file,'wb') as f:
          w = csv.writer(f)
          w.writerow(['ID','Condition','MajorityOrder','NounPosition','markerNum', 'markerCase', 'Phase', 'Meaning', 'NounItem','InputSentence', 'Selection', 'TypedResponse', 'SpokenResponse', 'AnswerQuest', 'Trial', 'date&time'])
      f.close()
    else:
...
def write_outputCSV(path,ID,condition,condition_order,noun_position, markerNum, markerCase, phase, meaning_filename, sentence, selection, typed_response, repsonse_questionnaire,trial):
    #add the nounItem for each target trial
    ...
    with open(file, 'a') as f:
        w=csv.writer(f)
        w.writerow([ID,condition,condition_order,noun_position, markerNum, markerCase, phase, meaning_filename, nounItem, sentence, selection, typed_response, '', repsonse_questionnaire, trial, time.asctime()])
    f.close()
...
def pre_experiment_screen(myWin,path):
  ...
  next_page=visual.ImageStim(myWin, image=path+'keys/return_key.png', units='norm', size=[0.2,0.3],pos=[0.8,-0.8],flipHoriz=False,flipVert=False,autoLog=True)
...
def explicit_instructions(myWin,path, dict_sentences, dict_images):
  image_chef_pl = path+'U39_stims/images/small/chef_two.png'
  image_chef_sg = path+'U39_stims/images/small/chef.png'
  image_chef_nom = path+'U39_stims/images/small/kick_chef_pl_burglar_pl.png'
  image_chef_nom2 = path+'U39_stims/images/small/kick_burglar_pl_chef_pl.png'
  chef_pl = sentence2text(dict_sentences[get_key(image_chef_pl,dict_images)])
  chef_sg = sentence2text(dict_sentences[get_key(image_chef_sg,dict_images)])
  chef_nom = sentence2text(dict_sentences[get_key(image_chef_nom,dict_images)][0])
  chef_nom2 = sentence2text(dict_sentences[get_key(image_chef_nom2,dict_images)][0])
  instrText = visual.TextStim(myWin,pos=[0,0], units='norm', height=0.085, wrapWidth = 1.5, color='black',text="""In this experiment, Kwin will teach you some aspects of their native language Tak Pisan. \n\nIn particular, they will teach you: \n\n 1) to mark singular and plural on nouns that refer to characters, and \n\n 2) to mark the role of a character in an event, that is, whether the character is performing an action or receiving it.""")
  next_page=visual.ImageStim(myWin, image=path+'keys/return_key.png', units='norm', size=[0.2,0.3],pos=[0.8,-0.8],flipHoriz=False,flipVert=False,autoLog=True)
  instrText.draw(); next_page.draw(); myWin.flip()
  while True:
    keys = event.getKeys()
    if 'return' in keys: break
    if 'escape' in keys: core.quit()
  instrText = visual.TextStim(myWin,pos=[0,0.6], units='norm', height=0.085, wrapWidth=1.5, color='black',text="""For example, you will learn that '""" + chef_pl + "' means 'cooks' and '"+ chef_sg + "' means 'cook' instead:")
  instrStim_sg=visual.ImageStim(myWin, image=image_chef_sg, units='norm',pos=[0.45,0],flipHoriz=False,flipVert=False,autoLog=True)
  instrStim_pl=visual.ImageStim(myWin, image=image_chef_pl, units='norm',pos=[-0.45,0],flipHoriz=False,flipVert=False,autoLog=True)
  sentence_sg=visual.TextStim(myWin, color='black', units='norm', height=0.085, pos=[0.45,-0.4],text=chef_sg+" (cook)")
  sentence_pl=visual.TextStim(myWin, color='black', units='norm',height=0.085, pos=[-0.45,-0.4],text=chef_pl+" (cooks)")
  for i in [instrText, instrStim_pl,instrStim_sg, sentence_sg, sentence_pl,next_page]:
    i.draw()
  myWin.flip()
  while True:
    keys = event.getKeys()
    if 'return' in keys: break
    if 'escape' in keys: core.quit()
  instrText = visual.TextStim(myWin,pos=[0,0.6], units='norm', height=0.085, wrapWidth=1.5, color='black',text="""You will also learn that  '""" + chef_nom.split(' ')[1] + "' refers to cooks 'who are performing an action'. Instead,  '"+ chef_pl + "' alone refers to cooks 'who are receiving an action':")
  instrStim_nom2=visual.ImageStim(myWin, image=image_chef_nom2, units='norm',pos=[0.45,0],flipHoriz=False,flipVert=False,autoLog=True)
  instrStim_nom=visual.ImageStim(myWin, image=image_chef_nom, units='norm',pos=[-0.45,0],flipHoriz=False,flipVert=False,autoLog=True)
  sentence_nom2=visual.TextStim(myWin, color='black', units='norm', height=0.085, pos=[0.45,-0.5],text="     "+chef_nom2+" \n(the burglars kick the cooks)")
  sentence_nom=visual.TextStim(myWin, color='black', units='norm',height=0.085, pos=[-0.45,-0.5],text="     "+chef_nom+" \n(the cooks kick the burglars)")
  for i in [instrText, instrStim_nom,instrStim_nom2, sentence_nom, sentence_nom2,next_page]:
    i.draw()
  myWin.flip()
  while True:
    keys = event.getKeys()
    if 'return' in keys: break
    if 'escape' in keys: core.quit()
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=900, color='black',text="""Press return when you are ready to meet Kwin!""")
  next_page=visual.ImageStim(myWin, image=path+'keys/return_key.png', units='norm', size=[0.2,0.3],pos=[0.8,-0.8],flipHoriz=False,flipVert=False,autoLog=True)
  #next_tag= visual.TextStim(myWin, color='black', units='norm', pos=[0.45,-0.6], text='Start')
  instrText.draw()
  next_page.draw()
...
def instructions_NounTraining(myWin,path):
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=900, color='black',text="""Gude! \n\nMy name is Kwin. I am here to help you learn my native language. It is called: Tak Pisan.  \n\nIf you can learn to speak just like me, then you will have learned Tak Pisan successfully. \n\nWe will start with something easy.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
  playSound('instructions/intro1',path)
  myWin.setMouseVisible(True) 
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=900, color='black',text="""First, I will teach you the names of some characters in Tak Pisan.  \n\nI will show you the pictures and say their name in my language. \n\nYou must REPEAT the names aloud after me. \n\nPress the return key when you're ready to get started.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False) 
...
def instructions_simplePhraseTraining(myWin,path):
  ...
  while True:
    ...
    if 'escape' in keys: core.quit()
  event.clearEvents()
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=950, color='black',text="""In this part of your training, I will show you pictures as before, but this time, what I say will be a bit more complex.  \n\nThe pictures you will see will look familiar, but they will illustrate some other features of Tak Pisan.  \n\nLOOK at the pictures, LISTEN, and REPEAT aloud after me. \n\nPress the return key when you are ready to start.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False) 
...
def instructions_comprehension(myWin, path):
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=950, color='black',text="""Ok. Now it is time to test your understanding of Tak Pisan. \n\nI will say a sentence in Tak Pisan and you will have to SELECT THE PICTURE it describes. \n\nUse the keyboard to select your answer. \n\nPress the return key when you're ready to start the test.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_complexPhraseComprehension(myWin, path):
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=950, color='black',text="""Ok. Now it is time to further test your understanding of Tak Pisan. \n\nI will say a sentence in Tak Pisan and you will have to SELECT THE PICTURE it describes. \n\nUse the keyboard to select your answer. \n\nPress the return key when you're ready to start the test.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_PhraseTesting(myWin,path,condition):
  if condition == 'regularisation':
    instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=950, color='black',text="""Ok, you are now ready for your final test: it is time for you to speak again!  \n\nI will show you pictures, and you must provide their corresponding descriptions in Tak Pisan.""")
  elif condition == 'extrapolation':
    instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=950, color='black',text="""Ok, you are now ready for your final test: it is time for you to speak again!  \n\nI will show you pictures, and although you have not heard the descriptions for all of them before, you will have to describe them using the Tak Pisan you already know.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_questionnaire(myWin, path):
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=950, color='black',text="""Well done! \n\nThat is all the Tak Pisan you will learn today.  Thank you very much for your interest in my language.  \n\nBefore you leave, I would like to ask you some questions about certain features of Tak Pisan. \n\nPress the return key to start answering the questions.""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def meaning_marker(myWin, markerInfo):
  marker = markerInfo[0]
  question = "What does '%s' describe in Tak Pisan?"%(marker)
...
def rate_freq(myWin, markerNum, markerCase, noun_position):
  if noun_position == 'initial':
    ...
    direction = 'after'
  question = "During your training in Tak Pisan, when both '%s' and '%s' appeared together, what percentage of the time did '%s' appear %s '%s'?" % (markerNum,markerCase,markerNum,direction,markerCase)
    ...
    while event.getKeys(keyList=['return'])==[]:
        ...
        for l in letterlist:
            if shifton:
                ...
                #otherwise, take the last letter off the string
        #continually redraw text onscreen until return pressed
        response = visual.TextStim(wptr, text=text+'-',color="black",units = 'norm', pos = [0,0] )
        response.draw()
        instructions.draw()
        wptr.flip()
    return text
