...
def draw_fixation(myWin):
  fixation = visual.TextStim(myWin, color='gray', units='norm', pos=[0,0], font='Hiragino Sans W2', text='') 
...
def dict_sentence2text():
  ...
  text_signals = [u"ケルラ", u"ナグラ", u"サスラ", u"ソギナ", u"ダクメ", u"ネチビ", u"タソヌ", u'セヒ', u'ギト', u'ヨザ']
...
def sentence2text_gap(sentence):
  ...
  sentence_list = sentence.split('_')
  sentence_list = [word for word in sentence_list]#REDUNDANT
  gaps_list = [u'＿'*len(dict_sentence_text[word]) for word in sentence_list]
...
def choose_foilNoun(noun):
  object_signals = [u'ソギナ', u'ダクメ', u'ネチビ', u'タソヌ']
...
def training_trial(myWin, path, meaning, sentence):
  ...
  texty=visual.TextStim(myWin, color='black', units='norm', pos=[0,-0.75], font='Hiragino Sans W2', text=sentence2text(sentence))
  draw_fixation(myWin)
...
def noun_selection_trial(myWin,path, meaning, sentence):
  ...
  stimy=visual.ImageStim(myWin, image=meaning, units='norm',pos=[0,0.1],flipHoriz=False,flipVert=False,autoLog=True)
  texty_noun=visual.TextStim(myWin, color='black', units='norm', pos=positions[0][0], font='Hiragino Sans W2', text=noun)
  texty_foil=visual.TextStim(myWin, color='black', units='norm', pos=positions[1][0], font='Hiragino Sans W2', text=foil_noun)
...
def noun_production_trial(myWin, meaning, sentence):
  stimy=visual.ImageStim(myWin, image=meaning, units='norm',pos=[0,0.1],flipHoriz=False,flipVert=False,autoLog=True)
  texty=visual.TextStim(myWin, color='black', units='norm', pos=[0,-0.75], font='Hiragino Sans W2', text=sentence2text_gap(sentence)+'?')
  draw_fixation(myWin)
  stimy.draw()
  texty.draw()
  myWin.flip() 
      ...
      for l in letterlist:
          if shifton:
              ...
              if len(l) > 1:
                  ...
                  text += l
                  if re.search(r'(k|s|t|n|h|m|y|r|w|g|z|d|b|p)?(a|i|u|e|o|nn)', text):
                    kana = re.search(r'(k|s|t|n|h|m|y|r|w|g|z|d|b|p)?(a|i|u|e|o|nn)', text).group()
                    text = re.sub(kana, E2J[kana], text)
              #otherwise, take the last letter off the string
      #continually redraw text onscreen until return pressed
      typedLetters = len(text)
      response = visual.TextStim(myWin, text=verb+text+gaps[typedLetters:], color="black", units = 'norm', pos = [0,-0.75], font='Hiragino Sans W2')        
      # if len(text)==0:
      #   initial_texty.draw()
      # elif len(text)!=0: 
      response.draw()
...
def feedback_trial(myWin,path,meaning,sentence,response,phase,noun_position):
  ...
  stimy=visual.ImageStim(myWin, image=meaning, units='norm',pos=[0,0.1],flipHoriz=False,flipVert=False,autoLog=True)
  texty=visual.TextStim(myWin, color='black', units='norm', pos=[0,-0.75], font='Hiragino Sans W2', text=written_sentence)
  draw_fixation(myWin)
  stimy.setAutoDraw(True)
...
def phrase_testing_trial(myWin, meaning, sentence):
  ...
  stimy=visual.ImageStim(myWin, image=meaning, units='norm',pos=[0,0.1],flipHoriz=False,flipVert=False,autoLog=True)
  texty=visual.TextStim(myWin, color='black', units='norm', pos=[0,-0.75], font='Hiragino Sans W2', text=fillIn_sentence)
  draw_fixation(myWin)
  stimy.draw()
  texty.draw()
  myWin.flip()
...
def pre_experiment_screen(myWin,path):
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text=u"""準備ができたら、リターンキーを押してください。""")
...
def instructions_NounTraining(myWin,path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text=u"""こんにちは！　\n\nMy name is Maki. I am here to help you learn my native language. It is called: Yurani.  \n\nIf you can learn to speak just like me, then you will have learned Yurani successfully. \n\nWe will start with something easy.""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text=u"""こんにちは！私の名前はマキです。\n\nこれからあなたが私の母語を学ぶお手伝いをします。\n\nその言語は、ユラニ語と呼ばれています。\n\nもし私みたいに話せるようになれば、\n\nユラニ語をマスターしたということになります。\n\n簡単なことから始めましょう。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
  playSound('instructions/intro1',path)
  myWin.setMouseVisible(True) 
  #instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text="""First, I will teach you the names of some characters in Yurani.  \n\nI will show you the pictures of the characters and say their name in my language. \n\nYou must REPEAT the names aloud after me. \n\nPress the return key when you're ready to get started.""")
  instrText = visual.TextStim(myWin,pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text=u"""まず、ユラニ語のキャラクターの名前を教えます。\n\nキャラクターの写真を見せて、ユラニ語で名前を言います。\n\n私に続いてその名前を声に出して言ってください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False) 
...
def instructions_NounSelection(myWin, path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text="""Ok. Now it is time to take a short test on what you have learned so far. \n\nI will show you pictures of characters and you will have to select their correct names in Yurani. \n\nUse the keyboard to SELECT THE CORRECT NAME. \n\nPress the return key when you're ready to start the test. """)
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=900, color='black', font='Hiragino Sans W2', text=u"""オッケーです。それでは、\n\nあなたがこれまで学んだ内容をテストしてみましょう。\n\nこれからキャラクターの写真を見せるので、\n\nキーボードを使って、\n\nユラニ語で正しいと思う名前を選んでください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_simplePhraseTraining(myWin,path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Well done! You have made it through the first tests!  \n\nPress the return key to hear about your next task.""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""いいですね！最初のテストが終わりました！\n\nリターンキーを押して、次のトレーニングに進んでください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
  ...
  while True:
    ...
    if 'escape' in keys: core.quit()
  event.clearEvents()
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""In this part of your training, I will show you pictures as before, but this time, what I will say will be a bit more complex.  \n\nThe pictures you will see will look familiar, but they will illustrate some other features of Yurani.  \n\nLOOK at the pictures, LISTEN, and REPEAT aloud after me. \n\nPress the return key when you are ready to start.""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""このトレーニングでは、\n\n前と同じように写真を見せますが、今回は少しだけ複雑です。\n\nこれから見せる写真は既に馴染みがあるかもしれませんが、\n\nユラニ語の新たな特徴を教えてくれます。\n\n写真をよく見て、私が話すことをよく聞いて、\n\n私に続いて声に出して言ってください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False) 
...
def instructions_comprehension(myWin, path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Ok. Now it is time to further test your understanding of Yurani. \n\nI will say a sentence in Yurani and you will have to SELECT THE PICTURE it describes. \n\nUse the keyboard to select your answer. \n\nPress the return key when you're ready to start the test.""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""オッケーです。それでは、\n\nあなたのユラニ語の理解をテストしてみましょう。\n\n私がユラニ語である文を言うので、\n\nキーボードを使って、\n\nその文が表している写真を選んでください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_simplePhraseTyping(myWin, path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Ok. Now it is time to take another short test on what you have learned.  \n\nThis time I will show you pictures and you will have to TYPE IN their corresponding descriptions in Yurani.   \n\nTo make this task easier, you will be able to see the blanks you need to fill in to complete each description. \n\nUse the keyboard to FILL IN THE BLANKS and press the return key to submit your answer. \n\nPress the return key when you're ready to start typing!""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""オッケーです。\n\nあなたがこれまで学んだ内容を更にテストしてみましょう。\n\nこれから写真を見せるので、\n\nユラニ語でその写真を表す文を入力してください。\n\n空欄を埋めれば、文が完成するようになっていますので、\n\n簡単にできるはずです。\n\nキーボードを使って文を入力し、\n\nリターンキーを押して答えを提出してください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_complexPhraseTraining(myWin, path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Good job! Now, in this last part of your training, I will show you pictures as before, but what I say will be even more complex. \n\nYou will see pictures you are already familiar with, as well as new pictures that will illustrate further features of Yurani. \n\nLOOK at the pictures, LISTEN, and REPEAT aloud after me. \n\nPress the return key when you are ready to start.""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""よくできました！\n\nトレーニングも終わりが近づいてきました。\n\n前と同じように写真を見せますが、内容はさらに複雑になります。\n\n既に馴染みがある写真だけではなく、\n\nユラニ語の新たな特徴を示す新しい写真も出てきます。\n\n写真をよく見て、私が話すことをよく聞いて、\n\n私に続いて声に出して言ってください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_complexPhrasetyping(myWin,path,condition):
  if condition == 'regularisation':
    #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Well done! Now, before you move on to the final test, I want to make sure you are ready for the challenge with one more simple task. \n\nI will show you pictures and you will have to type in their corresponding descriptions in Yurani.  \n\nAs before, use the keyboard to TYPE IN THE DESCRIPTIONS and press the return key to submit your answers. \n\nPress the return key when you're ready to start the test.""")
    instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""いいですね！\n\n最後のテストに挑戦する前に、その準備ができているか、\n\n簡単なテストで確認してみたいと思います。\n\nこれから写真を見せるので、\n\nユラニ語でその写真を表す文を入力してください。\n\n前と同じように、キーボードを使って文を入力し、\n\nリターンキーを押して答えを提出してください。\n\n準備ができたら、リターンキーを押してください。""")
  elif condition == 'extrapolation':
    ...
    instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""いいですね！\n\n最後のテストに挑戦する前に、その準備ができているか、\n\n簡単なテストで確認してみたいと思います。\n\nこれから写真を見せるので、\n\nまだユラニ語でその写真を表す文を教えていませんが、\n\nこれまで学んだユラニ語を使って文を入力してください。\n\n前と同じように、キーボードを使って文を入力し、\n\nリターンキーを押して答えを提出してください。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def instructions_PhraseTesting(myWin,path,condition):
  if condition == 'regularisation':
    #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Ok, you are now ready for your final test: it is time for you to speak again!  \n\nI will show you pictures, and you must provide their corresponding descriptions in Yurani.""")
    instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""オッケーです。\n\nそれでは、最後のテストを始めましょう。\n\nあなたがユラニ語を話す番です！\n\nこれから写真を見せるので、\n\nユラニ語でその写真を表す文を声に出して言ってください。""")
  elif condition == 'extrapolation':
    ...
    instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""オッケーです。\n\nそれでは、最後のテストを始めましょう。\n\nあなたがユラニ語を話す番です！\n\nこれから写真を見せるので、\n\nまだユラニ語でその写真を表す文を教えていませんが、\n\n既に知っているユラニ語を使って、\n\nその写真を表す文を声に出して言ってください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
  playSound('instructions/%s_complexPhraseTesting1'%(condition),path)
  myWin.setMouseVisible(True) 
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""As in previous tasks, I will help you by providing some words but this time you will have to COMPLETE THE DESCRIPTIONS ORALLY.  \n\nSpeak clearly into the microphone in front of you and press the return key to submit your answer. \n\nYou will be provided with feedback every now and then to help you refresh your memory. \n\nPress the return key when you are ready to start speaking!""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""前のテストと同じように、\n\nいくつかの語が与えられていますが、\n\n今回は口頭で文を完成させてください。\n\nマイクに向かってはっきりと話し、\n\nリターンキーを押して答えを提出してください。\n\n思い出す手がかりとして、\n\nときどきフィードバックが与えられます。\n\n準備ができたら、リターンキーを押してください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False) 
...
def instructions_questionnaire(myWin, path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""Well done! \n\nThat is all the Yurani you will learn today.  Thank you very much for your interest in my language.  \n\nBefore you leave, I would like to ask you some questions about certain features of Yurani. \n\nPress the return key to start answering the questions.""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""お疲れさまでした！\n\n今回学んでもらうユラニ語は以上です。\n\nユラニ語に興味を持ってくれて本当にありがとう。\n\n最後に、ユラニ語の特徴について、\n\nいくつか質問をしたいと思います。\n\nリターンキーを押して、質問に答えてください。""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def post_experiment_screen(myWin,path):
  #instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text="""That is all! \n\nYou are now finished. \n\nArigato, yoi hi wo!!!""")
  instrText = visual.TextStim(myWin, pos=(0,0), units='pix', height=40, wrapWidth=950, color='black', font='Hiragino Sans W2', text=u"""以上です！全て終了しました。\n\nありがとう、良い一日を！！！""")
  instrText.draw(); myWin.flip()
  myWin.setMouseVisible(False)
...
def meaning_marker(myWin, markerInfo):
  marker = markerInfo[0]
  question = u"ユラニ語で、'%s'は何を表していますか？\n\n（カタカナで入力してください）"%(marker)
...
def order_markerNoun(myWin, markerInfo):
  marker = markerInfo[0]
  question = u"あなたが学んだ文で、'%s'はキャラクターの名前の前と後ろどちらに現れていましたか？\n\n（カタカナで入力してください）"%(marker)
  answer = getAndShow(myWin,question)
...
def order_markers(myWin, markerNum, markerCase, noun_position):
  if noun_position == 'initial':
    ...
    direction = u'前'
  question = u"'%s'と'%s'が両方キャラクターの名前の%sに現れた時、'%s'は'%s'の前、'%s'の後ろ、あるいは両方に現れていましたか？\n\n（カタカナで入力してください）"%(markerNum, markerCase,direction,markerNum, markerCase, markerCase)
  answer = getAndShow(myWin,question)
...
def rate_freq(myWin, markerNum, markerCase, noun_position):
  if noun_position == 'initial':
    ...
    direction = u'後ろ'
  question = u"ユラニ語を学んでいる間、'%s'と'%s'が同時に現れた時、どれくらいの割合で'%s'が'%s'の%sに現れていましたか？\n\n（カタカナで入力してください）" % (markerNum,markerCase,markerNum,markerCase,direction)
  ...
  item = visual.TextStim(myWin, color='DimGray', units='norm', pos=[0,0.75], font='Hiragino Sans W2', text= question, wrapWidth = 1.5)
...
def getAndShow(wptr,instructionsToShow):
    text=""
    shifton=0 # allows caps and ?'s etc
    instructions = visual.TextStim(wptr, text=instructionsToShow, color="DimGray", units='norm', pos=[0,0.6], wrapWidth = 1.5, font='Hiragino Sans W2')
    while event.getKeys(keyList=['return'])==[]:
        ...
        for l in letterlist:
            if shifton:
                ...
                #otherwise, take the last letter off the string
        #continually redraw text onscreen until return pressed
        response = visual.TextStim(wptr, text=text+'_', color="black", units = 'norm', pos = [0,0], font='Hiragino Sans W2')
        response.draw()
        instructions.draw()
        wptr.flip()
    return text
